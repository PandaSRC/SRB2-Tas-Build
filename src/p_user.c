...
//		return true;
	...
	else if (tails->mo && (player->cmd.buttons & BT_ATTACK))
	{
		angle = R_PointToAngle2(thiscam->x, thiscam->y, tails->mo->x, tails->mo->y);
		focusaiming = -0x05000000;
	}
	...
	// Zenya's TAS Build. All instances of this else if have to do with the Tails cam hack.
	else if (tails->mo && (player->cmd.buttons & BT_ATTACK))
	{
		angle = R_PointToAngle2(thiscam->x, thiscam->y, tails->mo->x, tails->mo->y);
		focusaiming = -0x05000000;
	...
	if (!(twodlevel || (mo->flags2 & MF2_TWOD)) && !(player->powers[pw_carry] == CR_NIGHTSMODE)) // This block here is like 90% Lach's work, thanks bud
	{
		if (!resetcalled && ((thiscam == &camera && cv_cam_adjust.value) || (thiscam == &camera2 && cv_cam2_adjust.value)))
		{
			...
			&& thiscam->ceilingz - thiscam->floorz >= P_GetPlayerHeight(player)) // Check that camera's sector is large enough for the player to fit into, at least
			{
				...
				{
					slopez += max(thiscam->floorz - tails->mo->z - tails->mo->height, 0);
				}
	...
	{
		x = tails->mo->x - FixedMul(FINECOSINE((tails->mo->angle>>ANGLETOFINESHIFT) & FINEMASK), distxy);
		y = tails->mo->y - FixedMul(FINESINE((tails->mo->angle>>ANGLETOFINESHIFT) & FINEMASK), distxy);
	}
	else
	{
	...
	else
	{
		if (mo->eflags & MFE_VERTICALFLIP)
			...
			z = tails->mo->z + 64*FRACUNIT;
		else
			z = mo->z + pviewheight + camheight + distz;
	}

	// move camera down to move under lower ceilings
	if (tails->mo && (player->cmd.buttons & BT_ATTACK))
		newsubsec = R_PointInSubsectorOrNull(((tails->mo->x>>FRACBITS) + (thiscam->x>>FRACBITS))<<(FRACBITS-1), ((tails->mo->y>>FRACBITS) + (thiscam->y>>FRACBITS))<<(FRACBITS-1));
	else
...
*/
	...
	{
		viewpointx = tails->mo->x;
		viewpointy = tails->mo->y;
	}
	else
	{
...
*/
	...
	if (mo->eflags & MFE_VERTICALFLIP)
		...
		angle = R_PointToAngle2(0, thiscam->z, dist, (sign ? sign->floorz : tails->mo->z) + P_GetPlayerHeight(tails));
	else
